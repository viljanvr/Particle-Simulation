...
std::vector<Force::JacobianEntry> QuadraticDragForce::getJv() {
    ...
    for (Particle *p: m_particles) {
        ...
        // two rows of v * normalized(v)^T
        Vec2f o1 = Vec2f(v[0] * v[0], v[0] * v[1]) / norm(v);
        Vec2f o2 = Vec2f(v[1] * v[0], v[1] * v[1]) / norm(v);
        // two rows of jacobian
        Vec2f j1 = Vec2f(- 0.5 * m_strength * o1 + 0.5 * m_strength * Vec2f(norm(v), 0));
        Vec2f j2 = Vec2f(- 0.5 * m_strength * o1 + 0.5 * m_strength * Vec2f(0, norm(v)));
