...
Collision CircularCollisionObject::compute_collision_details(Particle *p) const {
    ...
    if (norm(p->m_Position - m_Origin) - m_Radius > 0) {
        Vec2f collision_normal = p->m_Position - m_Origin;
        return Collision{p, collision_normal / norm(collision_normal), 0.0};
    }
    ...
    Vec2f collision_normal = (p->m_PreviousPosition + t * d) - m_Origin;

    return Collision{p, collision_normal / norm(collision_normal), 1.0 - t};
}
